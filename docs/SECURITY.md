...
- **Post-install tampering**: install-time SHA256 hashes are recorded in state; `scuta doctor --audit` re-hashes every managed binary and reports drift (modified outside scuta), and resolves each tool on PATH to flag shadowing: a different binary earlier on PATH that would execute instead of the verified one (critical), or scuta's bin directory missing from PATH entirely (warning).
